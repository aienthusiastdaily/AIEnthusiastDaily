...
app = typer.Typer(
    add_completion=False, no_args_is_help=True, help="Prepare public-safe repo snapshots."
)
console = Console()
...
class PublicExportConfig:
    ...
    def from_file(cls, path: str | Path) -> PublicExportConfig:
        ...
        config = cls(
            ...
            secret_scan_exclude=tuple(
                str(item) for item in verification.get("secret_scan_exclude", [])
            ),
        )
        config.validate()
        return config
    ...
    def validate(self) -> None:
        ...
        patterns = (*self.include, *self.exclude, *self.forbidden_paths, *self.secret_scan_exclude)
...
def _validate_relative_pattern(pattern: str) -> None:
    ...
    if path.is_absolute() or ".." in path.parts:
        raise ValueError(f"public export patterns must be relative and stay inside repo: {pattern}")
...
def _iter_included_files(source_root: Path, config: PublicExportConfig) -> list[Path]:
    files: set[Path] = set()
    for match in source_root.rglob("*"):
        if not match.is_file():
            continue
        relative_path = _to_posix_relative(match, source_root)
        if _is_excluded(relative_path, config.exclude):
            continue
        if any(_matches_pattern(relative_path, pattern) for pattern in config.include):
            files.add(Path(relative_path))
    ...
    *,
    ...
    if apply:
        ...
        for rel_path in plan.files:
            ...
            shutil.copy2(plan.source_root / rel_path, target_file)
...
def _render_plan(payload: dict[str, Any]) -> None:
    table = Table(title="Public Export")
    ...
    table.add_row("Verification", "valid" if verification["valid"] else "failed")
    console.print(table)
...
def plan_command(
    config_path: str = typer.Option(
        DEFAULT_CONFIG_PATH, "--config", help="Public export TOML config."
    ),
    ...
    _emit_json(payload) if json_output else _render_plan(payload)
...
def sync_command(
    ...
    apply: bool = typer.Option(
        False, "--apply", help="Actually write files. Without this, only dry-run."
    ),
    clean: bool = typer.Option(
        True, "--clean/--no-clean", help="Remove destination contents except .git."
    ),
    json_output: bool = typer.Option(False, "--json", help="Emit machine-readable JSON."),
    verbose: bool = typer.Option(False, "--verbose", help="Enable debug logging."),
) -> None:
